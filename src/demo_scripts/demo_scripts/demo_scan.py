...
class DemoScan(Node):
    """A ROS2 ControllerSet service client that requests scan promo."""
    ...
    def __init__(self):
        ...
        self.object_detection_on = self.create_client(
            srv_type=DetectObjectsOn, srv_name="/turn_object_detection_on"
        )

        self.object_detection_off = self.create_client(
            srv_type=DetectObjectsOff, srv_name="/turn_object_detection_off"
        )
        ...
        self.service_client_controller_position_set = self.create_client(
            srv_type=ControllerPositionSet, srv_name="/controller_position_set"
        )
        ...
        while not self.service_client_controller_set.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                f"service {self.service_client_controller_set.srv_name} not available, waiting..."
            )
        while not self.service_client_controller_position_set.wait_for_service(
            timeout_sec=1.0
        ):
            self.get_logger().info(
                f"service {self.service_client_controller_position_set.srv_name} not available, waiting..."
            )

        # Object detection
        while not self.object_detection_on.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                f"service {self.object_detection_on.srv_name} not available, waiting..."
            )
        while not self.object_detection_off.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                f"service {self.object_detection_off.srv_name} not available, waiting..."
            )
        ...
        match self.current_case:
            ...
            case 1:
                ...
                if self.dectect_objects_on_future is None:
                    ...
                    if (
                        self.dectect_objects_on_future is not None
                        and not self.dectect_objects_on_future.done()
                    ):
                        self.dectect_objects_on_future.cancel()
                        self.get_logger().warn(
                            "Object detection on cancelled is camera node still running? "
                        )

                    self.dectect_objects_on_future = (
                        self.object_detection_on.call_async(request_object_detection_on)
                    )
                    self.dectect_objects_on_future.add_done_callback(
                        self.after_object_detection_on
                    )
            ...
            case 2:
                ...
                self.object_detection_timer = self.create_timer(
                    timer_period, self.object_detection_timer_callback
                )
            ...
            case 3:
                ...
                try:
                    ...
                    self.get_logger().error(f"Error popping object from list: {e}")
                    self.current_case = 1
                    self.initial_object_list = []
            # uses grasp block at given position
            case 33:
                ...
                if (
                    self.controller_position_set_future is not None
                    and not self.controller_position_set_future.done()
                ):
                    self.controller_position_set_future.cancel()
                    self.get_logger().warn(
                        "Controller position set on cancelled is controller node still running? "
                    )

                self.controller_position_set_future = (
                    self.service_client_controller_position_set.call_async(
                        request_object_pick
                    )
                )
                self.controller_position_set_future.add_done_callback(
                    self.after_controller_position_set
                )
    ...
    def controller_set_config(self, config: int, callback):
        ...
        if (
            self.controller_set_future is not None
            and not self.controller_set_future.done()
        ):
            self.controller_set_future.cancel()
            self.get_logger().warn(
                f"Config {config} cancelled is controller node still running? "
            )

        self.controller_set_future = self.service_client_controller_set.call_async(
            request_manipulator_config
        )
    ...
    def record_detected_object_position(self, msg: DetectedObjects):
        if self.store_initial_object_list is True:
    ...
    def object_detection_timer_callback(self):
        self.store_initial_object_list = False
        if self.dectect_objects_off_future is None:
            request_object_detection_off = DetectObjectsOff.Request()

            if (
                self.dectect_objects_off_future is not None
                and not self.dectect_objects_off_future.done()
            ):
                self.dectect_objects_off_future.cancel()
                self.get_logger().warn(
                    "Object detection off cancelled is camera node still running? "
                )

            self.dectect_objects_off_future = self.object_detection_off.call_async(
                request_object_detection_off
            )
            self.dectect_objects_off_future.add_done_callback(
                self.after_object_detection_off
            )
    ...
    def clustering(self, eps, list, min_samples=1):
        ...
        try:
            ...
            self.get_logger().error(f"Error occurred while clustering: {e}")
            results = None
        return results
...
def main(args=None):
    try:
        ...
        omega_operator_controller_node = DemoScan()
